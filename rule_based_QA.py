...
class RuleBasedQA:
    ...
    def answer_dev(self, save=True):
        ...
        with open('dev.csv', 'wb') as csv_file:
            ...
            # for i in range(2,4):
                ...
                qs_entities = self.ner_process(dev_qs)
    ...
    def remove_entity_in_qs(self, qs_entities, entities):
        valid_entities = []
        for entity in entities:
            entity_words = entity[0].split()
            for word in entity_words:
                word = word.lower()
                if self.bdp.lemmatize(word) not in qs_entities:
